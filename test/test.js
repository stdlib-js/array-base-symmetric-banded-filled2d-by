...
tape( 'the function returns a filled symmetric banded nested array', function test( t ) {
	var expected;
	var actual;

	expected = [ [ 'beep', 'beep' ], [ 'beep', 'beep' ] ];
	actual = filled2dBy( 2, 1, null, constantFunction( 'beep' ) );
	t.deepEqual( actual, expected, 'returns expected value' );

	expected = [ [ 0, 1, 2 ], [ 1, 2, 3 ], [ 2, 3, 4 ] ];
	actual = filled2dBy( 3, 2, -1, clbk );
	t.deepEqual( actual, expected, 'returns expected value' );

	expected = [ [ 0, 1, -1 ], [ 1, 2, 3 ], [ -1, 3, 4 ] ];
	actual = filled2dBy( 3, 1, -1, clbk );
	t.deepEqual( actual, expected, 'returns expected value' );

	expected = [ [ 0, -1, -1 ], [ -1, 2, -1 ], [ -1, -1, 4 ] ];
	actual = filled2dBy( 3, 0, -1, clbk );
	t.deepEqual( actual, expected, 'returns expected value' );
...
tape( 'the function returns an empty outer array if provided a first argument equal to zero', function test( t ) {
	...
	expected = [];
	actual = filled2dBy( 0, 1, null, constantFunction( 'beep' ) );
	t.deepEqual( actual, expected, 'returns expected value' );
...
tape( 'the function supports providing an execution context', function test( t ) {
	...
	expected = [ [ 'beep', 'beep' ], [ 'beep', 'beep' ] ];
	actual = filled2dBy( 2, 1, null, clbk, ctx );
...
tape( 'the function invokes a provided callback function with one argument', function test( t ) {
	...
	idx = [];
	actual = filled2dBy( 3, 1, '', clbk );

	expected = [ [ 'beep', 'beep', '' ], [ 'beep', 'beep', 'beep' ], [ '', 'beep', 'beep' ] ];
	t.deepEqual( actual, expected, 'returns expected value' );
